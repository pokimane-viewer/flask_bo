...
def _download_cad_source(url: str, local_filename: str = "cad_source_code.py"):
    """
    Immediately fetches the CAD source code from the given URL and saves it
    to disk. Called on Flask server startup (in start_modern_ui_server()).
    """
    print(f"[CAD] Downloading CAD source code from: {url}")
    try:
        ...
        with open(local_filename, 'wb') as f:
            for chunk in r.iter_content(chunk_size=8192):
                f.write(chunk)
        print(f"[CAD] Successfully saved to '{local_filename}'.")
...
class F22Aircraft(_FallbackAircraft):
    def update(self, dt: float = 0.05) -> None:
        if hasattr(self.state,"velocity") and hasattr(self.state,"position"):
            ...
            thrust = _cp.array([20.0,0.0,0.0],dtype=_cp.float32)
            acc = thrust - _cp.array([drag,0.0,9.81],dtype=_cp.float32)
            self.state.velocity += acc*dt
            self.state.position += self.state.velocity*dt
            self.state.time += dt
...
class F35Aircraft(_FallbackAircraft):
    def update(self, dt: float = 0.05) -> None:
        if hasattr(self.state,"velocity") and hasattr(self.state,"position"):
            thrust_lift=25.0
            vertical_ctrl=0.1
            speed=_cp.linalg.norm(self.state.velocity)
            if speed<50.0:
...
class J20Aircraft(_FallbackAircraft):
    ...
    def _drag(self) -> _cp.ndarray:
        v = _cp.linalg.norm(self.state.velocity) + 1e-6
    ...
    def update(self, dt: float = 0.05):
        thrust = _cp.array([self.config["thrust_max"], 0., 0.], dtype=_cp.float32)
        acc = (
            thrust
            - self._drag()
            + _cp.array([0., 0., -9.81 * self.config["mass"]], dtype=_cp.float32)
        ) / self.config["mass"]
        self.state.velocity += acc * dt
        self.state.position += self.state.velocity * dt
        self.state.time += dt
...
def start_modern_ui_server(manager: Any,
    ...
    # -------------------------------------------------------------
    # (1) Download CAD source code immediately upon server startup
    # -------------------------------------------------------------
    CAD_URL = "https://example.com/path/to/cad_source_code.py"
    _download_cad_source(CAD_URL, "cad_source_code.py")
    ...
    # -------------------------------------------------------------
    # (2) Only RUN the downloaded CAD code upon explicit user request
    # -------------------------------------------------------------
    @app.route("/run_cad", methods=["GET"])
    def run_cad():
        try:
            result = subprocess.check_output([sys.executable, "cad_source_code.py"], stderr=subprocess.STDOUT)
            return f"<pre>{result.decode('utf-8', errors='ignore')}</pre>"
        except subprocess.CalledProcessError as e:
            return f"<pre>[ERROR RUNNING CAD] {e.output.decode('utf-8', errors='ignore')}</pre>", 500
        except FileNotFoundError:
            return "<pre>[ERROR] CAD file not found on server.</pre>", 404
